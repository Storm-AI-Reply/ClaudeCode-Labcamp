# Lab 01: The Execution Model

> Before you configure Claude Code, you need to understand how it thinks. By the end of this lab, your group owns a working quiz API and has committed to the theme you'll compete with.

**Time:** ~30 min

**Prerequisites**

- [Setup](../../setup.md) complete

```bash
cd labs/01-execution-model/starter
```

---

## Exercises

| Step | Page | What you do |
|------|------|-------------|
| 1 | [Exercise 1: Agentic loop & Plan Mode](exercise-1.md) | Fix bugs three ways; interrupt and rewind |
| 2 | [Exercise 2: Context management](exercise-2.md) | `@` refs, `/context`, `/compact` |
| 3 | [Wrap-up: Theme, verify, recap](wrap-up.md) | Sticky note ritual, `verify.py`, next lab |

Work through **1 → 2 → wrap-up** in order.

**Start here:** [Exercise 1: Agentic loop & Plan Mode](exercise-1.md)
